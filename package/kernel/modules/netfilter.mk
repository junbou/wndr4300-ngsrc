...
define KernelPackage/ipt-snatp2p
  SUBMENU:=$(NF_MENU)
  TITLE:=SNATP2P target
  KCONFIG:=$(KCONFIG_IPT_SNATP2P)
  FILES:=$(foreach mod,$(IPT_SNATP2P-m),$(LINUX_DIR)/net/$(mod).$(LINUX_KMOD_SUFFIX))
  AUTOLOAD:=$(call AutoLoad,42,$(notdir $(IPT_SNATP2P-m)))
  DEPENDS:= kmod-ipt-core +kmod-ipt-conntrack @TARGET_wndr4300
endef

define KernelPackage/ipt-snatp2p/description
 Netfilter (IPv4) kernel modules for SNATP2P targets
 Includes:
 - SNATP2P
endef

$(eval $(call KernelPackage,ipt-snatp2p))

define KernelPackage/ipt-hairpin
  SUBMENU:=$(NF_MENU)
  TITLE:=HAIRPIN target
  KCONFIG:=$(KCONFIG_IPT_HAIRPIN)
  FILES:=$(foreach mod,$(IPT_HAIRPIN-m),$(LINUX_DIR)/net/$(mod).$(LINUX_KMOD_SUFFIX))
  AUTOLOAD:=$(call AutoLoad,43,$(notdir $(IPT_HAIRPIN-m)))
  DEPENDS:= kmod-ipt-core +kmod-ipt-conntrack @TARGET_wndr4300
endef

define KernelPackage/ipt-hairpin/description
 Netfilter (IPv4) kernel modules for HAIRPIN targets
 Includes:
 - HAIRPIN
endef

$(eval $(call KernelPackage,ipt-hairpin))

define KernelPackage/ipt-dnatp2p
  SUBMENU:=$(NF_MENU)
  TITLE:=DNATP2P target
  KCONFIG:=$(KCONFIG_IPT_DNATP2P)
  FILES:=$(foreach mod,$(IPT_DNATP2P-m),$(LINUX_DIR)/net/$(mod).$(LINUX_KMOD_SUFFIX))
  AUTOLOAD:=$(call AutoLoad,43,$(notdir $(IPT_DNATP2P-m)))
  DEPENDS:= kmod-ipt-core +kmod-ipt-conntrack @TARGET_wndr4300
endef

define KernelPackage/ipt-dnatp2p/description
  Netfilter (IPv4) kernel modules for DNATP2P targets
  Includes:
  - DNATP2P
endef

$(eval $(call KernelPackage,ipt-dnatp2p))

define KernelPackage/ipt-log
  SUBMENU:=$(NF_MENU)
  TITLE:=LOG match
  KCONFIG:=$(KCONFIG_IPT_LOG)
  FILES:=$(foreach mod,$(IPT_LOG-m),$(LINUX_DIR)/net/$(mod).$(LINUX_KMOD_SUFFIX))
  AUTOLOAD:=$(call AutoLoad,43,$(notdir $(IPT_LOG-m)))
  DEPENDS:= kmod-ipt-core +kmod-ipt-conntrack @TARGET_wndr4300
endef

define KernelPackage/ipt-log/description
  Netfilter (IPv4) kernel modules for LOG match
  Includes:
  - LOG
endef

$(eval $(call KernelPackage,ipt-log))
