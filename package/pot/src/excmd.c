 ...
 * parameter:
 * 	mtd - device name of the partition, like as "/dev/mtd3"
 * 	blk - which block of this partition will be erased, start from 0
 **/
int erase_block(char *mtd, int blk)
{
	int devfd;
	...
	mtdEraseInfo.start = blk * mtdInfo.erasesize;
	mtdEraseInfo.length = mtdInfo.erasesize;
	ioctl(devfd, MEMUNLOCK, &mtdEraseInfo);
...
int read_block(char *mtd, int blk, void *buf, int len)
{
	int devfd;

	if (buf == NULL || len <NAND_FLASH_BLOCKSIZE)
		return 1;
	...
	lseek(devfd, blk * NAND_FLASH_BLOCKSIZE, SEEK_SET);
	read(devfd, buf, len);
	close(devfd);
...
int write_block(char *mtd, int blk, void *buf, int len)
{
	int devfd;
	...
	if (devfd >= 0) {
		...
		dfp("write data into flash fail!\n");
	}

	return 0;
}

int read_page(char *mtd, int blk, int pag, void *buf, int len)
{
	int devfd;

	if (buf == NULL || len <= NAND_FLASH_PAGESIZE)
		return 1;
	...
	lseek(devfd, blk * NAND_FLASH_BLOCKSIZE + pag * NAND_FLASH_PAGESIZE, SEEK_SET);
	read(devfd, buf, len);
	close(devfd);
...
int write_page(char *mtd, int blk, int pag, void *buf, int len)
{
	int devfd;
