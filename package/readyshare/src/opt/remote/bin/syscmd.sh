...
do_getinfo()
{
    ...
    # add hostname
    DATA="${DATA}<hostname>WNDR4300</hostname>"
    # add identity
    # DATA="${DATA}`${SYS_PREFIX}/bin/getinfo.pl`"
	DATA="${DATA}<identity><arch>mips</arch><model>WNDR4300</model><osname>OpenWRT</osname><osver>1.0.4.68</osver><serial>`remote_smb_conf -get_device_serial_number`</serial></identity>"
    # add enclosure
