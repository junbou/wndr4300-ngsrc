...
do_register_fake()
{
	...
	# Laser: restart the process of this module
	...
	comm_post "${DATA}" && {
		if [ "xSUCCESS" == "x$COMM_RESULT" ]; then
			...
			$nvram commit >/dev/null
			#echo "Register to Server Successfully"
...
do_register()
{
	# first time register should not include name/pass,otherwise it would fail.
	# This is the server problem of Netgear. We do to fit it.
	COMM_EXEC="${SYS_PREFIX}/bin/curl --basic -k --user : --url ${URL}"
	do_register_fake $1 $2

	factory_mac_addr=`echo $(${nvram} get wan_factory_mac) | tr '[a-z]' '[A-Z]'`
	factory_mac_addr=`echo $factory_mac_addr | sed 's/://g'`
	hostname=$(${nvram} get wan_hostname)
	NAS_NAME="$hostname""_$factory_mac_addr"
	NAS_PASS="!$factory_mac_addr!"
	COMM_EXEC="${SYS_PREFIX}/bin/curl --basic -k --user ${NAS_NAME}:${NAS_PASS} --url ${URL}"
...
do_unregister()
{
	...
	echo "Connect to Server fail, Please check inernet connection" > /dev/console
	return $ERROR
}
...
add_remote_network()
{
	...
	eval COMM_RESULT="${REMOTE_REQUEST}" || {
		echo error executing request "${REMOTE_REQUEST}" > /dev/console
	}
	return $OK
}
start()
{
	do_register "duyong" "D121323123"
	echo "start register" > /dev/console	
