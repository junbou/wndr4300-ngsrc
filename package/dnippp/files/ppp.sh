...
if [ "\$(config get wan_proto)" = "pptp" -a "\$(config get wan_pptp_dns_assign)" = "1" ]; then
	if [ "x\$staticdns1" != "x" ]; then
		...
		if [ "x\$(config get pptp_gw_static_route)" != "x" ]; then
			...
			/sbin/route -A inet add -net "\$staticdns1" netmask 255.255.255.255 gw "\$(config get pptp_gw_static_route)" eth1
		else
			echo nameserver "\$staticdns1" > /tmp/resolv.conf
		fi
	fi
	if [ "x\$staticdns2" != "x" ];then
		...
		if [ "x\$(config get pptp_gw_static_route)" != "x" ]; then
			...
			/sbin/route -A inet add -net "\$staticdns2" netmask 255.255.255.255 gw "\$(config get pptp_gw_static_route)" eth1
		else
			if [ "x\$staticdns1" != "x" ]; then
				echo nameserver "\$staticdns2" >> /tmp/resolv.conf
			else
	...
	if [ "x\$staticdns1" != "x" ]; then
                ...
                if [ "x\$(config get l2tp_gw_static_route)" != "x" ]; then
                        echo nameserver "\$staticdns1" >> /tmp/resolv.conf
                        /sbin/route -A inet add -net "\$staticdns1" netmask 255.255.255.255 gw "\$(config get l2tp_gw_static_route)" eth1
                else
                        echo nameserver "\$staticdns1" > /tmp/resolv.conf
                fi
        fi
        if [ "x\$staticdns2" != "x" ];then
                ...
                if [ "x\$(config get l2tp_gw_static_route)" != "x" ]; then
                        echo nameserver "\$staticdns2" >> /tmp/resolv.conf
                        /sbin/route -A inet add -net "\$staticdns2" netmask 255.255.255.255 gw "\$(config get l2tp_gw_static_route)" eth1
                else
                        if [ "x\$staticdns1" != "x" ]; then
                                echo nameserver "\$staticdns2" >> /tmp/resolv.conf
                        else
...
if [ "\$(config get wan_proto)" = "pptp" -a "x\$(config get pptp_gw_static_route)" = "x" ]; then
	cat /tmp/dhcpc_resolv.conf >> /tmp/resolv.conf
elif [ "\$(config get wan_proto)" = "l2tp" -a "x\$(config get l2tp_gw_static_route)" = "x" ]; then
	cat /tmp/dhcpc_resolv.conf >> /tmp/resolv.conf
...
setup_interface_ppp() {
	...
	if [ "$($CONFIG get wan_pptp_dns_assign)" = "1"  -a "x$($CONFIG get pptp_gw_static_route)" != "x" ]; then
	...
	if [ "x$($CONFIG get wan_pptp_connection_id)" != "x" ]; then
        ...
        if [ "$($CONFIG get wan_l2tp_dns_assign)" = "1" -a "x$($CONFIG get l2tp_gw_static_route)" != "x" ]; then
