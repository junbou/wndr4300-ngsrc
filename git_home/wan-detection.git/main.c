...
int send_arprequst()
{
	...
	wan_inaddr.s_addr = wan_subnet+1;
			
	//for debug
	struct in_addr test_inaddr1;
	test_inaddr1.s_addr = wan_subnet+1;
	char cmd_3[256];
	...
	test_inaddr2.s_addr = wan_brodcast -1;
	char cmd_4[256];
	...
	{
		for(cur_addr=wan_subnet+1;cur_addr<wan_brodcast;cur_addr++)
		{
		...
		while(timeout.tv_sec >= 0)
		{
			...
			if(alen>=42)
			{
				if(arp_packet.ar_op == htons(ARPOP_REPLY)&&(memcmp(ap_autodection_wanaddress1,arp_packet.ar_tip,4)==0))
				{
					//for debug 
					char cmd_4[256];
					snprintf(cmd_4,256,"echo '[AP autodetection]  recv arp response %d.%d.%d.%d' > /dev/console",arp_packet.ar_sip[0],arp_packet.ar_sip[1],arp_packet.ar_sip[2],arp_packet.ar_sip[3]);
					system(cmd_4);
...
void AP_autodection(char *dutMac)
{
	...
	if(is_DHCP=DHCP_Detection(&bpa,dutMac)){
		...
		if( ((memcmp(ap_autodection_wanaddress1, ap_autodection_wanaddress2, 4) == 0) 
		    ...
		    || ((memcmp(ap_autodection_wanaddress1, ip_zero, 4) != 0) && (memcmp(ap_autodection_wanaddress2, ip_zero, 4) == 0 ))
		    || ((memcmp(ap_autodection_wanaddress2, ip_zero, 4) != 0) && (memcmp(ap_autodection_wanaddress1, ip_zero, 4) == 0 )) )
		{
			//for one US cable modem
			//the behavior is that cable modem only can assign 1 ip address to router
			system("echo '[AP autodetection] the ip address is the same' > /dev/console");
		...
		{
			//if the ip address is not the same, detection is done, show ap mode & router mode optional page
			system("echo '[AP autodetection] the ip address is different' > /dev/console");
...
void AP_autodection_lan(char *dutMac)
{
	int bpa;
	int is_DHCP;
