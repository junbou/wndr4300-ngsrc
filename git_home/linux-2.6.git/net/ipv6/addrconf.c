...
void addrconf_dad_failure(struct inet6_ifaddr *ifp)
{
	...
	if (idev->cnf.accept_dad == 2 && !idev->cnf.disable_ipv6) {
	...
	if (idev->cnf.accept_dad == 3 && !idev->cnf.disable_ipv6) {
		struct in6_addr addr;
		struct in6_addr mask;
		struct in6_addr newaddr;
		struct inet6_ifaddr *newifp;

		printk(KERN_INFO "%s: Need to randomly generate another IPv6 address!\n",
			ifp->idev->dev->name);
		memset(&addr, 0x0, sizeof(struct in6_addr));
		addr.s6_addr32[0] = htonl(0xfe800000);

		memset(&mask, 0x0, sizeof(struct in6_addr));
		mask.s6_addr32[0] = htonl(0xffffffff);
		mask.s6_addr32[1] = htonl(0xffffffff);

		if (ipv6_masked_addr_cmp(&addr, &mask, &ifp->addr) == 0) {
			newaddr = ifp->addr;
			get_random_bytes(&newaddr.s6_addr32[2],8);

			newifp = ipv6_add_addr(idev, &newaddr, 64, IFA_LINK, IFA_F_PERMANENT);
			if (!IS_ERR(newifp)) {
				addrconf_dad_start(newifp, 0);
				in6_ifa_put(newifp);
			}
