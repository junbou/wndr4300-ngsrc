...
last_lba(struct block_device *bdev)
{
	if (!bdev || !bdev->bd_inode)
       ...
       //common disk use 512 sector size
       //but the large volume disk may use 4096 sector size
       int n = bdev_logical_block_size(bdev);
       if(4096==n)
       {
          return (bdev->bd_inode->i_size >> 12) - 1ULL;
       }
       else
...
efi_partition(struct parsed_partitions *state, struct block_device *bdev)
{
	...
	for (i = 0; i < le32_to_cpu(gpt->num_partition_entries) && i < state->limit-1; i++) {
		...
		put_partition(state, i+1, le64_to_cpu(ptes[i].starting_lba) * ssz,
