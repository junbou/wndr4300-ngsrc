...
static void ath_set_tuning_caps_in_nand(void)
{
        typedef struct {
                u_int8_t        pad[0x28],
        ...
        size = sizeof(ar9300_eeprom_t);

        ret = nand_read(nand, (loff_t)WLANCAL, &size, (u_char *)sectorBuff);

        ar9300_eeprom_t *eep = (ar9300_eeprom_t *)sectorBuff;
#if 0
	int i;
	printf("\n cal data \n");
	for (i=0; i < size; i++){
		printf("0x%02x ", *(unsigned char *)(sectorBuff+i));
	}
	printf("---END ---\n");
#endif
        uint32_t        val;
