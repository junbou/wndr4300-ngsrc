...
#if defined(CONFIG_ETH_SOFT_LED) && (defined(CONFIG_WNR1000V4) || defined(CONFIG_HW29763847P16P64))
    eth_led_control();
#endif
    return (linkCount);
...
void eth_led_control(void)
{
	...
	for (i = 0 ; i < 5 ; i ++) {
		...
		phyAddr = ATHR_PHYADDR(i);
		if (athrs26_phy_is_link_alive(i)) {
			ar7240_reg_wr (AR7240_GPIO_FUNC, (ar7240_reg_rd(AR7240_GPIO_FUNC) | (1 << (i + 3))));
			s26_wr_phy(phyAddr,0x19,0x0);
			...
			if (cnt == 0) {
				s26_wr_phy(phyAddr,0x19,(s26_rd_phy(i,0x19) | (0x3c0)));
				s26_wr_phy(phyAddr,0x18,(0x4));
				continue;
			}
			if (athrs26_phy_speed(i) == _10BASET) {
				bRateTab = BlinkRateTable_10M;
			}
			while (bRateTab++) {
				s26_wr_phy(phyAddr,0x18,((bRateTab->timeOn << 12)|
	                           (bRateTab->timeOff << 8)));
				s26_wr_phy(phyAddr,0x19,(s26_rd_phy(i,0x19) & ~(0x280)));
				break;
			}
		} else {
			ar7240_reg_wr (AR7240_GPIO_FUNC, (ar7240_reg_rd(AR7240_GPIO_FUNC) & (~(1 << (i + 3)))));
