...
# error Unknown CPU type
	...
	case IH_COMP_LZMA:
		printf ("   Uncompressing %s ... ", name);
#ifdef CONFIG_HW29763654P16P128
		i = lzma_unzip((char *)data,len,(char*)ntohl(hdr->ih_load));
		if (i == 0)
#else
...
int do_loadn_dniimg ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int dev;
	ulong offset, addr, cnt, kernel_partition_size;
	image_header_t *hdr;
	nand_info_t *nand;
	...
	cnt = nand->oobblock;
	if ( nand_read(nand, offset, &cnt, (u_char *) addr) ) {
	...
	kernel_partition_size = (((2 * sizeof(image_header_t) + ntohl(hdr->ih_size))
	           / CFG_FLASH_SECTOR_SIZE) + 1) * CFG_FLASH_SECTOR_SIZE;
	if (kernel_partition_size > CFG_IMAGE_LEN) {
		printf("\n** Bad partition size, kernel : 0x%x **\n", kernel_partition_size);
		return 1;
	}

	cnt = kernel_partition_size;
	if ( nand_read(nand, offset, &cnt, (u_char *) addr) ) {
	...
	hdr = (image_header_t *) (addr + kernel_partition_size - sizeof(image_header_t));
	root_fs_size = ntohl(hdr->ih_size);
	if (root_fs_size > CFG_IMAGE_LEN - kernel_partition_size) {
		printf("\n** Bad partition size, kernel: 0x%x, rootfs: 0x%x **\n", kernel_partition_size, root_fs_size);
		return 1;
	}

	cnt = root_fs_size;
	if (nand_read(nand,
	    (offset + get_len_incl_bad (nand, offset, kernel_partition_size)),
	    &cnt, (u_char *)(addr + kernel_partition_size))) {
...
int do_chk_dniimg ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	ulong addr;
