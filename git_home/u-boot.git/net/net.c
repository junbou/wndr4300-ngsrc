...
NetLoop(proto_t protocol)
{
	...
	eth_halt();
#ifdef CONFIG_NET_MULTI
#if defined(CFG_VITESSE_73XX_NOPHY) || defined(CONFIG_WNR1000V2) || defined(CONFIG_WNR1100) || defined(CONFIG_WNR2200) || defined(CONFIG_WNR2000V3) || defined(CONFIG_WNR1000V4) || defined(CONFIG_HW29763904P4P32)
	/*
...
#ifdef FIRMWARE_RECOVER_FROM_TFTP_SERVER
#ifndef CFG_NO_FLASH
#if defined(CONFIG_WNR1000V2) || defined(CONFIG_WNR1100) || defined(CONFIG_WNR2200) || defined(CONFIG_WNR2000V3) || defined(CONFIG_WNR1000V4) || defined(CONFIG_HW29763847P16P64) || defined(CONFIG_WNDR3700V3) || defined(CONFIG_HW29763904P4P32) || defined(CONFIG_HW29764037P8P64)
...
#ifdef CFG_NO_FLASH /* NAND products */
void update_data(ulong addr, int data_size, ulong target_addr_begin, size_t target_addr_len, int send_nmrp_alive)
{
	ulong passed;
	int offset_num;
	uchar *src_addr;
	...
	size_t write_size = CFG_FLASH_SECTOR_SIZE;
	int ret = 0;
	...
	for (offset_num = 0;
	     offset_num < (((data_size - 1) / CFG_FLASH_SECTOR_SIZE) + 1);
	     offset_num++) {
		while (nand_block_isbad(&nand_info[0], target_addr)) {
			printf("Skipping erasing bad block at 0x%08lx\n", target_addr);
			target_addr += CFG_FLASH_SECTOR_SIZE;
		}
		if (target_addr >= target_addr_begin + target_addr_len)
			goto bad_nand;

		ret = nand_erase(&nand_info[0], (loff_t)(target_addr),
		        (ulong)CFG_FLASH_SECTOR_SIZE);
		printf("nand erase: off %x, size %x: %s\n", (ulong)(target_addr),
		        (ulong)CFG_FLASH_SECTOR_SIZE, ret ? "ERROR" : "OK");

		src_addr = addr + offset_num * CFG_FLASH_SECTOR_SIZE;
		write_size = CFG_FLASH_SECTOR_SIZE;
		ret = nand_write(&nand_info[0],
		        (loff_t) target_addr, (ulong*)&write_size, (u_char *)src_addr);
		printf("nand write %x : %x bytes written, %s\n",
		        target_addr, write_size, ret ? "ERROR" : "OK");
...
{
	if (get_len_incl_bad(&nand_info[0], (loff_t)CFG_IMAGE_ADDR_BEGIN,
	    (size_t)firmware_size) > (size_t)CFG_IMAGE_LEN)
	{
		printf("** FAIL !! too many bad blocks, no enough space for firmware image.\n");
		return;
	}
	update_data(addr, firmware_size, CFG_IMAGE_ADDR_BEGIN, CFG_IMAGE_LEN, 1);
#ifdef CFG_NMRP
	if(NmrpState != 0)
		return;
#endif
...
#ifndef CFG_NO_FLASH /* NOR products */
void update_data(ulong addr, int data_size, ulong target_addr_begin, int send_nmrp_alive)
{
	if (data_size <= 0) {
		printf("Incorrect data size\n");
		return;
	...
	target_addr = target_addr_begin;
	for (offset_num = 0;
	     offset_num < ((data_size / CFG_FLASH_SECTOR_SIZE) + 1);
	     offset_num++) {
		/* erase 64K */
		flash_sect_erase(target_addr_begin +
...
void update_firmware(ulong addr, int firmware_size)
{
	...
	update_data(addr, firmware_size, CFG_IMAGE_ADDR_BEGIN, 1);
#else
...
#if defined(CFG_NMRP) && (defined(CFG_SINGLE_FIRMWARE) || defined(CONFIG_WNR1000V2) || defined(CONFIG_WNR2200) || defined(CONFIG_WNR2000V3))
