...
#define SIGCONFCHANGE	12
#define SIGMYTRAP	8
#define CFG_CHANGE_COUNT_FILE	"/tmp/config/cfgHistChangeCount"
...
#ifdef	SIGCONFCHANGE 
RETSIGTYPE
SnmpdWebChange(int a)
{
	FILE* fp;
    	++my_cfgHistChangeCount;
    	send_easy_trap(SNMP_TRAP_CONFCHANGE, 0);
	fp=fopen(CFG_CHANGE_COUNT_FILE,"w");
	if(fp)
	{
		fprintf(fp,"%u",my_cfgHistChangeCount);
		fclose(fp);
	}
	else
		printf("can not open %s for write\n",CFG_CHANGE_COUNT_FILE);
//	system("/bin/save-config");
    signal(SIGCONFCHANGE, SnmpdWebChange);
}
#endif
...
void my_snmptrap(void)
{
	int trapType = 0;
	FILE *fp;

	fp = fopen("/tmp/config/snmp_trap_type", "r");
	if (fp){
		fscanf(fp, "%d", &trapType);
		fclose(fp);
		switch(trapType){
			case 3:  // 1.3.6.1.4.1.388.13.2.3.3
				send_easy_trap(SNMP_TRAP_ETHACLVIO, 0);
				break;
			case 4:  // 1.3.6.1.4.1.388.13.2.3.4
				send_easy_trap(SNMP_TRAP_FWUPGRADEFAIL, 0);
				break;
			case 5:  // 1.3.6.1.4.1.388.13.2.3.5
				send_easy_trap(SNMP_TRAP_CONFUPGRADEFAIL, 0);
				break;
			case 6:  // 1.3.6.1.4.1.388.13.2.3.6
				send_easy_trap(SNMP_TRAP_INVALIDTEXTCONF, 0);
				break;
			case 7:  // 1.3.6.1.4.1.388.13.2.3.7
				send_easy_trap(SNMP_TRAP_TIMEADOPTFAIL, 0);
				break;
			case 8:  // 1.3.6.1.4.1.388.13.2.3.8
				send_easy_trap(SNMP_TRAP_RFTHRESHOLDTHRO, 0);
				break;
			case 9:  // 1.3.6.1.4.1.388.13.2.3.9
				send_easy_trap(SNMP_TRAP_RFAVGRETRIES, 0);
				break;
			case 10:  // 1.3.6.1.4.1.388.13.2.3.10
				send_easy_trap(SNMP_TRAP_PROCESSFAIL, 0);
				break;
			default:
				break;
		}
...
{
    my_snmptrap();
    //signal(SIGMYTRAP, SnmpdUserTrap);
}
#endif
...
#endif
{
    ...
    *argvptr = NULL;
...
#ifdef SIGCONFCHANGE
    DEBUGMSGTL(("signal", "registering SIGWEBCHANGE signal handler\n"));
    signal(SIGCONFCHANGE, SnmpdWebChange);
#endif
#ifdef SIGTERM
...
#ifdef SIGMYTRAP
    DEBUGMSGTL(("signal", "registering SIGMYTRAP signal handler\n"));
    signal(SIGMYTRAP, SnmpdUserTrap);
#endif
	...
	update_config();
	fp=fopen(CFG_CHANGE_COUNT_FILE,"r");
	if(fp)
	{
		fscanf(fp,"%u",&my_cfgHistChangeCount);
		fclose(fp);
	}
	else
	{
		my_cfgHistChangeCount=0;
		printf("can not open %s for read",CFG_CHANGE_COUNT_FILE);
	}
	...
	fp=fopen("/tmp/config/snmp_trap_type", "r");
	if( fp != NULL ){
		fscanf(fp, "%d", &trap_type);
		if( trap_type == 10 ){
			fscanf(fp, "%s", process);
			fclose(fp);

			if( strcmp(process, "snmpd") == 0 ){
				send_easy_trap(SNMP_TRAP_PROCESSFAIL, 0);

				fp = fopen("/tmp/config/snmp_trap_type", "w");
				if( fp ){
					putc(0, fp);
					fclose(fp);
				}
...
receive(void)
{
    ...
    reselect:
		...
		if(!my_isInACL)
		{
			send_easy_trap(SNMP_TRAP_ACLFAIL,0);
