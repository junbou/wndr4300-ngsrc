...
init_system_mib(void)
{
    ...
    snprintf(version_descr, sizeof(version_descr),
            "%s %s %s %s %s MIB=%s", utsName.sysname,
            utsName.nodename, utsName.release, utsName.version,
            utsName.machine, MIB_VERSION);
           ...
           int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    case UPTIME:
        ...
        return ((u_char *) & ulret);
    case SYSCONTACT:
	fp=fopen("/tmp/config/snmp_syscontact","r");
	if(fp)
	{
	    if(fscanf(fp,"%s",sysContact)<0)
		sysContact[0]=0;
	    fclose(fp);
	}
	else
		strcpy(sysContact,"Unknow");
        *var_len = strlen(sysContact);
        *write_method = writeSystem;
        return (u_char *) sysContact;
    case SYSTEMNAME:
	fp=fopen("/tmp/config/device_name","r");
        if(fp)
        {
            if(fscanf(fp,"%s",sysName)<0)
                sysName[0]=0;
            fclose(fp);
        }
	else
		strcpy(sysName,"Unknow");
        *var_len = strlen(sysName);
        *write_method = writeSystem;
        return (u_char *) sysName;
    case SYSLOCATION:
	fp=fopen("/tmp/config/snmp_syslocation","r");
        if(fp)
        {
            if(fscanf(fp,"%s",sysLocation)<0)
                sysLocation[0]=0;
            fclose(fp);
        }
	else
		strcpy(sysLocation,"Unknow");
            ...
            u_char * statP, oid * name, size_t name_len)
{
	...
	strcpy(command,"/usr/bin/cfgset --syscontact \"");
        break;
    case SYSTEMNAME:
        buf = sysName;
        oldbuf = oldsysName;
        setvar = &sysNameSet;
	strcpy(command,"/usr/bin/cfgset --device \"");
        break;
    case SYSLOCATION:
        buf = sysLocation;
        oldbuf = oldsysLocation;
        setvar = &sysLocationSet;
	strcpy(command,"/usr/bin/cfgset --syslocation \"");
        break;
    default:
