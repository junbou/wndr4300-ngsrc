...
int my_isSkipCheckACL;
int my_isInACL;
unsigned long 	my_aclViolations;
unsigned long	my_lastDeniedIpAddr;
/*
                     ...
                     u_char * packetptr, int length)
{
  ...
  if (netsnmp_ds_get_boolean(NETSNMP_DS_LIBRARY_ID, 
	...
	/*
	 * ACL check,add by josn
	*/
    my_isInACL=1;
    if(!my_isSkipCheckACL)
    {	
        if(!is_in_ACL(transport, opaque, olength))
        {
                my_isInACL=0;
		++my_aclViolations;
		...
		{
        		addr = (struct sockaddr_in *)transport->data;
    		}
		else
			;
		if(addr!=NULL)
		{
			my_lastDeniedIpAddr=addr->sin_addr.s_addr;
		}
		else
			my_isInACL=1;
		return -1;
