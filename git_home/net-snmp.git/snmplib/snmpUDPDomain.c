...
void updateACL()
{
	...
	fp=fopen("/etc/snmp/acl.conf","r");
        head=NULL;
	if(fp)
        {
                while(1)
                {
                        n=fscanf(fp,"%s %s",start,end);
                        if(n<=0)
                                break;
                        node=(ACL*)malloc(sizeof(ACL));
                        if(node)
                        {
                                node->start_ip=ntohl(inet_addr(start));
                                node->end_ip=ntohl(inet_addr(end));
                                node->next=NULL;
                                if(head==NULL)
                                        head=node;
                                else
	...
	{
		 snmp_log(LOG_ERR,
                     "get ACL fail : can not open /tmp/ACL \n");
	}	
	g_acl=head;
}

/*
	1 if in ACL, 0 if not in ACL
*/
int is_in_ACL(netsnmp_transport *transport, void *opaque, int olength)
{
	ACL *head;
        unsigned long src;
	...
	if (opaque != NULL && olength == sizeof(struct sockaddr_in)) 
		...
		from = (struct sockaddr_in *) transport->data;
	else
		return 0;
        src=ntohl(from->sin_addr.s_addr);
        for(head=g_acl;head;head=head->next)
        {
                if(src>=head->start_ip && src <=head->end_ip)
                {
                        break;
                }
        }
