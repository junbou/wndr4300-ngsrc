...
function change_serv_adva()
{
	...
	else
	{
		...
		var i=(new_qos_flag == 1) ? cf.apps.selectedIndex*2 : cf.apps.selectedIndex;
...
function check_qoslist_editnum(cf)
{
	...
	var count_select=0;
	var select_num;
	var num = (new_qos_flag == 1) ? 2 : 1; /* old qos design each policy have one rule, new qos design has two. */
	var array_num = (new_qos_flag == 1) ? qos_array_num/2 : qos_array_num;
	if( qos_array_num == num)
	{
		...
		{
			count_select++;
			select_num= (new_qos_flag == 1) ? (i*2+1) : i+1;
		}
	if(count_select==0)
	{
		alert("$port_edit");
...
function check_qos_apply(cf)
{
	...
	if(cf.qosEnable.checked == true && quick_qos ==1 && fast_lane == 2)
	{
