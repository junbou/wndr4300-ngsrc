...
function setANwlan_mode()
{
	...
	{ //Israel,Middle East(Turkey/Egypt/Tunisia/Kuwait) Middle East(Saudi Arabia) Africa
		...
		cf.opmode_an.options[1].text = wlan_mode_216;
	...
	{		// Middle East(Algeria/Syria/Yemen), this country do not support HT20 HT40,grayout channel
		...
		cf.opmode_an.options[0].text = wlan_mode_54;
		cf.opmode_an.options[1].text = wlan_mode_216;
		cf.opmode_an.options[2].text = wlan_mode_450;
		cf.opmode_an.options[0].value = "1";
		cf.opmode_an.options[1].value = "2";
	...
	for ( i = 0; i < tmp_array.length; i++ )
	{
		if ( tmp_array[i].indexOf("(DFS)") > -1 )
		{
			if ( 0 == hidden_dfs_channel && ( 1 == dfs_channel_router_flag ||
				( dfs_channel2_router_flag == 1 && ( index >= 2 && index <= 4) ) || //Australia, Canada, Europe
				( dfs_channel3_router_flag == 1 && (index == 6 || index == 10)) ) ) //Japan, United States
			{
...
*/
	...
	if(parent.mode_is_300 == 1 && wla_disablecoext != 1 && (cf.opmode_an.value!="1") && (cf.opmode_an.value!="2"))
	{
	   if(flad_op != true)
	     alert(msg);
