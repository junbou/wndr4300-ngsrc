...
#endif
			/* define the port yourselves */
			else if(strcasecmp(p, "CustomPort") == 0) {
				char *custom_port = (char)NULL;
				if((custom_port = strdup(q)) == (char *)NULL) {
					die("parse_config() -- strdup() failed");
				}

				if((atoi(custom_port))!=0){
					port=atoi(custom_port);
				}

				if(log_level > 0) {
					log_event(LOG_INFO, "Set CustomPort=\"%d\"\n", port);
				}
			}
