...
define Kernel/Prepare/Default
	...
	(cd $(KERNEL_BUILD_DIR); $(CP) $(GIT_HOME)/linux.git/* linux-$(LINUX_VERSION)/)
...
define Kernel/Configure/Default
	#rm -rf $(LINUX_DIR)-final
	#$(CP) $(LINUX_DIR) $(LINUX_DIR)-final
	case $(CUSTOMER) in \
		dni) \
			cat $(LINUX_CONFIG) > $(LINUX_DIR)/.config; \
		;; \
		netgear) \
			cat $(LINUX_CONFIG) > $(LINUX_DIR)/.config; \
		;; \
...
define Kernel/CompileModules/Default
	...
	+$(MAKE) $(KERNEL_MAKEOPTS) V=1 modules
endef
...
define Kernel/CompileImage/Default
	...
	+$(MAKE) $(KERNEL_MAKEOPTS) V=1 $(KERNELNAME)
